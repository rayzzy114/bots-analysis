...
from ..constants import COINS, DEFAULT_LINKS, LINK_LABELS, SELL_BTC_ADDRESS_DEFAULT
...
def build_admin_router(ctx: AppContext) -> Router:
    ...
    def apply_runtime_from_env() -> None:
        ...
        sell_btc_address = (env.get("SELL_BTC_ADDRESS") or "").strip()
        if sell_btc_address:
            ctx.settings.set_sell_btc_address(sell_btc_address)
    ...
    async def show_requisites_panel(message: Message) -> None:
        ...
        methods_lines = "\n".join(f"• {item}" for item in methods) if methods else "<i>Нет</i>"
        split_lines = ""
        if ctx.settings.requisites_mode == "split":
            split_map = ctx.settings.split_method_map()
            split_lines = "\n".join(
                f"• <b>{method}</b>: {row['bank']} / <code>{row['value']}</code>"
        ...
        text = (
            ...
            f"Sell BTC адрес: <code>{ctx.settings.sell_btc_address}</code>\n"
    ...
    @router.callback_query(F.data == "admin:req:edit_sell_btc")
    async def admin_req_edit_sell_btc(callback: CallbackQuery, state: FSMContext) -> None:
        user_id = callback_user_id(callback)
        msg = callback_message(callback)
        if user_id is None or msg is None or not ctx.is_admin(user_id):
            await callback.answer("Нет доступа", show_alert=True)
            return
        await callback.answer()
        await state.set_state(AdminState.waiting_admin_sell_btc_address)
        await msg.answer(
            "Отправьте BTC адрес для sell-заявок.\n"
            f"Текущее значение: <code>{ctx.settings.sell_btc_address}</code>"
        )

    @router.message(AdminState.waiting_admin_sell_btc_address)
    async def admin_sell_btc_input(message: Message, state: FSMContext) -> None:
        user_id = message_user_id(message)
        if user_id is None or not ctx.is_admin(user_id):
            await message.answer("Доступ запрещен.")
            return
        ...
        if len(value) < 20 or " " in value:
            await message.answer("Нужен корректный BTC адрес без пробелов.")
            return
        await persist_env_value("SELL_BTC_ADDRESS", value)
        if not ctx.settings.sell_btc_address.strip():
            ctx.settings.set_sell_btc_address(SELL_BTC_ADDRESS_DEFAULT)
        await state.clear()
        await message.answer("BTC адрес для sell обновлен.")
        await show_requisites_panel(message)
