...
from keyboards import kb_admin_reply, kb_close_select, kb_rating, kb_review, kb_unban
...
async def _send_source_aftercare_via_bot(bot: Bot, user_id: int, lang: str, is_alt_text: bool = False) -> None:
    ...
    temp_status = await bot.send_message(user_id, connecting)
...
async def _notify_admins_private(
    ...
    *,
    ...
    """Send client message to each admin's private chat with reply/ban buttons."""
    reply_markup = kb_admin_reply(user_id)
    for admin_id in ADMIN_IDS:
        try:
            if photo_id:
                await bot.send_photo(admin_id, photo_id, caption=caption, reply_markup=reply_markup)
            elif document_id:
                await bot.send_document(admin_id, document_id, caption=caption, reply_markup=reply_markup)
            elif video_id:
                await bot.send_video(admin_id, video_id, caption=caption, reply_markup=reply_markup)
            else:
                await bot.send_message(admin_id, text, reply_markup=reply_markup)
        except Exception:
            logger.exception("Failed to notify admin %d in private", admin_id)
...
async def client_source_reply(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    await _send_source_aftercare_via_bot(bot, user_id, lang)
...
async def client_text(message: Message, bot: Bot) -> None:
    ...
    if ADMIN_CHAT_ID:
        ...
        message_to_user[sent.message_id] = user_id
    await _notify_admins_private(bot, forward_text, user_id)
...
async def client_photo(message: Message, bot: Bot) -> None:
    ...
    if ADMIN_CHAT_ID:
        ...
        message_to_user[sent.message_id] = user_id
    await _notify_admins_private(bot, "", user_id, photo_id=photo.file_id, caption=caption)
...
async def client_document(message: Message, bot: Bot) -> None:
    ...
    if ADMIN_CHAT_ID:
        ...
        message_to_user[sent.message_id] = user_id
    await _notify_admins_private(
        bot, "", user_id, document_id=message.document.file_id, caption=caption,
    )
...
async def client_video(message: Message, bot: Bot) -> None:
    ...
    if ADMIN_CHAT_ID:
        ...
        message_to_user[sent.message_id] = user_id
    await _notify_admins_private(
        bot, "", user_id, video_id=message.video.file_id, caption=caption,
    )
