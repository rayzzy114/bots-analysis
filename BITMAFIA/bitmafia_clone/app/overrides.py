...
def apply_state_overrides(
    *,
    ...
    for text_field in ("text", "text_html", "text_markdown"):
        ...
        if live_rates_rub:
            value = _replace_live_rates(
                value,
                live_rates_rub,
                commission_percent=overrides.commission_percent,
                promo_discount_percent=overrides.promo_discount_percent,
            )
...
def _replace_live_rates(
    ...
    updated = "\n".join(out_lines)
    if multipliers:
        updated = _replace_payment_amounts(
            updated,
            multipliers[0],
    ...
    *,
    ...
    for line in lines:
        ...
        def repl_money(match: re.Match[str]) -> str:
            ...
            if "с учетом скидки" in normalized:
                discount_factor = 1.0 - max(float(promo_discount_percent), 0.0) / 100.0
                new_value *= max(discount_factor, 0.0)
...
def _format_like_source(value: float, source_token: str) -> str:
    _ = source_token
    return f"{_round_rub_int(value):,}".replace(",", " ")
...
def _round_rub_int(value: float) -> int:
    # User-visible monetary outputs must be integer rubles (half-up for positives).
    if value >= 0:
