...
def build_admin_router(ctx: AppContext) -> Router:
    ...
    async def show_requisites_panel(message: Message) -> None:
        btc_rate_text = "н/д (CoinGecko недоступен)"
        try:
    ...
    async def admin_rates(callback: CallbackQuery) -> None:
        ...
        try:
            ...
            await callback.answer("CoinGecko недоступен", show_alert=True)
            await msg.answer("⛔️ Не удалось получить актуальные курсы из CoinGecko. Попробуйте позже.")
            return
        lines = [
