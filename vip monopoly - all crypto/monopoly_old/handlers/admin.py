...
from db.settings import (
    ...
    get_btc_rates, get_xmr_rates, get_ltc_rates,
    get_commission, set_commission,
...
async def get_admin_panel_text(username: Optional[str]) -> str:
    ...
    xmr_usd, xmr_rub = await get_xmr_rates()
    ltc_usd, ltc_rub = await get_ltc_rates()
    ...
    text += (
        f"├ Курс BTC: <b>${btc_usd:,.2f}</b> / <b>{btc_rub:,.2f} ₽</b>\n"
        f"├ Курс XMR: <b>${xmr_usd:,.2f}</b> / <b>{xmr_rub:,.2f} ₽</b>\n"
        f"├ Курс LTC: <b>${ltc_usd:,.2f}</b> / <b>{ltc_rub:,.2f} ₽</b>\n"
...
async def admin_links(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback.message.edit_text("🔗 <b>Ссылки</b>", reply_markup=get_links_keyboard())
    await callback.answer()
...
async def admin_rates(callback: CallbackQuery, state: FSMContext):
    ...
    ltc_usd, ltc_rub = await get_ltc_rates()
    text = (
        f"💰 <b>Курсы (CoinGecko / Binance)</b>\n\n"
        f"├ BTC: <code>${btc_usd:,.2f}</code> / <code>{btc_rub:,.2f} ₽</code>\n"
        f"├ XMR: <code>${xmr_usd:,.2f}</code> / <code>{xmr_rub:,.2f} ₽</code>\n"
        f"╰ LTC: <code>${ltc_usd:,.2f}</code> / <code>{ltc_rub:,.2f} ₽</code>\n\n"
...
async def admin_change_rates(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "╰ <i>Введите новую ссылку на rates:</i>",
        reply_markup=kb.as_markup()
    )
    await callback.answer()
...
async def admin_change_sell_btc(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "╰ <i>Введите новую ссылку на sell_btc:</i>",
        reply_markup=kb.as_markup()
    )
    await callback.answer()
...
async def admin_change_news_channel(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "╰ <i>Введите новую ссылку на news_channel:</i>",
        reply_markup=kb.as_markup()
    )
    await callback.answer()
...
async def admin_change_operator(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "╰ <i>Введите новый username оператора:</i>",
        reply_markup=kb.as_markup()
    )
    await callback.answer()
...
async def admin_change_operator2(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "╰ <i>Введите новый username operator2:</i>",
        reply_markup=kb.as_markup()
    )
    await callback.answer()
...
async def admin_change_operator3(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "╰ <i>Введите новый username operator3:</i>",
        reply_markup=kb.as_markup()
    )
    await callback.answer()
...
async def admin_change_work_operator(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "╰ <i>Введите новый username work_operator:</i>",
        reply_markup=kb.as_markup()
    )
    await callback.answer()
