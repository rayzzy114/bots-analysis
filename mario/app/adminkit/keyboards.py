...
from .constants import CRYPTO_WALLET_COINS, LINK_LABELS
...
def kb_admin_requisites(commission_percent: float, mode_label: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🪙 Крипто-кошельки", callback_data="admin:req:wallets")],
...
def kb_admin_crypto_wallets(wallets: dict[str, str]) -> InlineKeyboardMarkup:
    rows = []
    for coin in CRYPTO_WALLET_COINS:
        value = str(wallets.get(coin, "")).strip()
        icon = "✅" if value else "❌"
        rows.append(
            [
                InlineKeyboardButton(
                    text=f"{icon} {coin}",
                    callback_data=f"admin:req:wallet:{coin}",
                )
            ]
        )
    rows.append([InlineKeyboardButton(text="↩️ Назад", callback_data="admin:requisites")])
