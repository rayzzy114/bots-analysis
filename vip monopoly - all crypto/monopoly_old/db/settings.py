...
DEFAULT_PAYMENT_METHODS = [
    ...
    {"name": "Любая карта", "requisites": "", "bank": ""},
    {"name": "Сбер ➡️ Сбер", "requisites": "", "bank": ""},
    {"name": "Перевод по номеру счета Сбербанк", "requisites": "", "bank": ""},
        ...
        if not await cursor.fetchone():
            await db.execute(
                """INSERT INTO settings (id, requisites, bank, payment_methods, commission, requisites_mode) 
                   VALUES (1, ?, ?, ?, ?, 0)""",
                (requisites or "", bank or "", json.dumps(DEFAULT_PAYMENT_METHODS, ensure_ascii=False), DEFAULT_COMMISSION)
            )
            await db.commit()
            return
...
async def get_xmr_rates() -> tuple:
    """Get XMR rates from CoinGecko, fallback to .env."""
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status == 200:
                    ...
                    if usd > 0 and rub > 0:
                        print(f"✅ CoinGecko XMR: ${usd}, ₽{rub:.0f}")
                        return (usd, rub)
    except Exception as e:
...
async def get_ltc_rates() -> tuple:
    """Get LTC rates from Binance, fallback to CoinGecko, fallback to .env."""
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
