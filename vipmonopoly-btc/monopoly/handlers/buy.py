...
async def process_amount(message: Message, state: FSMContext):
    ...
    amount_kind = _detect_amount_kind(raw_amount)
    if amount_kind is None:
        await message.answer("❌ Смешанный формат суммы. Укажите либо BTC, либо RUB.")
        return
    ...
    try:
        print(f"[AMOUNT] raw={raw_amount!r} parsed={amount_float} kind={amount_kind}")
