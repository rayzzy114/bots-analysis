...
def build_flow_router(ctx: AppContext) -> Router:
    ...
    async def buy_menu(message: Message, state: FSMContext) -> None:
        await state.clear()
        await answer_with_retry(message=message, text="Выберите валюту", reply_markup=kb_buy_menu())

    @router.message(F.text == "📉 Продать")
    async def sell_menu(message: Message, state: FSMContext) -> None:
        await state.clear()
        await answer_with_retry(message=message, text="Выберите валюту", reply_markup=kb_sell_menu())
    ...
    async def cabinet(message: Message) -> None:
        await answer_with_retry(
            ...
            text=(
                ...
                "https://telegram.me/your_bot?start=6131246501"
            ),
            reply_markup=kb_cabinet_menu(),
        )
