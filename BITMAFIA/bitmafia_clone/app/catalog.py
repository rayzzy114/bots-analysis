...
LINK_KEYWORDS: dict[str, tuple[str, ...]] = {
    ...
    "terms": ("услов", "terms"),
    "exchange": ("чистка btc", "mixer", "mixmafia"),
}
...
class FlowCatalog:
    ...
    def _resolve_inferred_action(
        ...
        *,
        ...
        if normalized == "Назад" and history and len(history) >= 2:
...
def _detect_about_state_id(states: dict[str, dict[str, Any]]) -> str:
    for sid, state in states.items():
        ...
        if not rows:
            continue
        texts = {str(btn.get("text") or "").strip() for row in rows for btn in row}
        if {"Отзывы", "Тарифы"} <= texts:
            return sid
    return ""
...
def _match_link_key(button_text: str) -> str | None:
    ...
    # Check review_form and reviews before generic LINK_LABELS to prevent
    # "Канал с отзывами" from being misclassified as "channel" via LINK_LABELS["channel"] = "Канал".
    for keyword in LINK_KEYWORDS["review_form"]:
        if keyword in text:
            return "review_form"

    for keyword in LINK_KEYWORDS.get("reviews", ()):
        if keyword in text:
            return "reviews"
