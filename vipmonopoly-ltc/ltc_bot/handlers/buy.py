...
from config import ADMIN_IDS, get_operator, get_operator2, get_operator3
...
async def get_ltc_rates():
    usd_price = await get_ltc_price_usd()
    rub_rate = await get_usd_rub_rate()
    final_rub_price = usd_price * rub_rate
    return usd_price, final_rub_price


@router.callback_query(F.data == "buy_ltc")
async def buy_ltc_handler(callback: types.CallbackQuery, state: FSMContext):
    await state.update_data(crypto_type="LTC")
    await state.set_state(Form.waiting_for_amount)

    text = (
        "<b>Покупка LTC</b>\n\n"
        "Укажите сумму в LTC или RUB:\n\n"
        "<code>Пример: 0.1 или 0,1 или 5000</code>\n\n"
...
async def process_amount(message: Message, state: FSMContext):
    ...
    minimum_ltc = MINIMUM_EXCHANGE_AMOUNT_RUB / rate_rub
    ...
    try:
        if amount_float <= 10:
            ...
            if crypto_amount < minimum_ltc:
                await message.answer(
                    f"❌ Минимальная сумма обмена: {MINIMUM_EXCHANGE_AMOUNT_RUB} руб. ({minimum_ltc:.4f} LTC)")
                return
        else:
        ...
        text = (
            f"Средний рыночный курс LTC: ${rate_usd:,.2f}, {await get_usd_rub_rate():,.2f} руб.\n\n"
            f"Вы получите: <b>{formatted_crypto} LTC</b>\n\n"
...
async def payment_method_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.delete()
    msg = await callback.message.answer("<b>Скопируйте и отправьте боту свой кошелек LTC</b>",
...
def generate_deal_text(total_to_pay: int, wallet: str) -> tuple:
    ...
    text = (
        ...
        f"пишите поддержке:\n"
        f"@{get_operator()} или @{get_operator2()}\n\n"
