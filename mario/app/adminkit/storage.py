...
from .constants import CRYPTO_WALLET_COINS, DEFAULT_PAYMENT_METHODS
...
class SettingsStore:
    def __init__(self, path: Path, default_commission: float, env_links: dict[str, str]):
        default_crypto_wallets = {coin: "" for coin in CRYPTO_WALLET_COINS}
        self.path = path
    ...
    def load(self) -> None:
        ...
        if isinstance(req_raw, dict):
            ...
            crypto_wallets_raw = req.get("crypto_wallets")
            if isinstance(crypto_wallets_raw, dict):
                parsed_wallets: dict[str, str] = {}
                for key, value in crypto_wallets_raw.items():
                    if not isinstance(key, str) or not isinstance(value, str):
                        continue
                    coin = key.strip().upper()
                    if coin not in CRYPTO_WALLET_COINS:
                        continue
                    parsed_wallets[coin] = value.strip()
                if parsed_wallets:
                    self.data["requisites"]["crypto_wallets"].update(parsed_wallets)
        ...
        self._normalize_crypto_wallets()
        self.save()
    ...
    def _normalize_crypto_wallets(self) -> None:
        wallets = self.data["requisites"]["crypto_wallets"]
        normalized: dict[str, str] = {}
        for coin in CRYPTO_WALLET_COINS:
            normalized[coin] = str(wallets.get(coin, "")).strip()
        self.data["requisites"]["crypto_wallets"] = normalized
    ...
    def crypto_wallet(self, coin: str) -> str:
        key = coin.strip().upper()
        if key not in CRYPTO_WALLET_COINS:
            return ""
        return self.data["requisites"]["crypto_wallets"].get(key, "")

    def crypto_wallets(self) -> dict[str, str]:
        self._normalize_crypto_wallets()
        return dict(self.data["requisites"]["crypto_wallets"])

    def set_crypto_wallet(self, coin: str, wallet: str) -> bool:
        key = coin.strip().upper()
        value = wallet.strip()
        if key not in CRYPTO_WALLET_COINS:
            return False
        if len(value) < 4 or len(value) > 256:
            return False
        self.data["requisites"]["crypto_wallets"][key] = value
        self.save()
        return True
