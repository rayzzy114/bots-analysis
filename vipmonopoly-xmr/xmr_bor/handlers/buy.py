...
CRYPTO_EXAMPLES = {
    ...
    "XMR": "0.5 или 0,5 или 5000",
    "LTC": "0.5 или 0,5 или 5000"
}
...
async def get_crypto_price_usd(crypto_type: str):
    try:
        async with aiohttp.ClientSession() as session:
            if crypto_type == "XMR":
...
async def get_rates(crypto_type: str):
...
async def buy_crypto_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    examples = CRYPTO_EXAMPLES.get(crypto_type, "0.1 или 5000")

    text = (
        f"<b>Покупка {crypto_type}</b>\n\n"
        f"Укажите сумму в {crypto_type} или RUB:\n\n"
        f"<code>Пример: {examples}</code>\n\n"
...
async def process_amount(message: Message, state: FSMContext):
    ...
    rate_usd, rate_rub = await get_rates(crypto_type)
    commission = await get_commission()

    min_crypto = MINIMUM_EXCHANGE_AMOUNT_RUB / rate_rub

    threshold = 10 if crypto_type != "BTC" else 1
    ...
    try:
        ...
        for i, method in enumerate(payment_methods):
            kb.button(text=f"{method['name']} ({formatted_rub} руб.)",
                      callback_data=f"pay_method_{i}_{total_to_pay}")
