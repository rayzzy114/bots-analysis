...
async def get_admin_panel_text(username: str | None) -> str:
    ...
    rate_usd, rate_rub = await get_ltc_rates()
    ...
    text += (
        f"├ Курс LTC: <b>${rate_usd:,.2f}</b> / <b>{rate_rub:,.2f} руб.</b> (авто)\n"
...
async def admin_change_rates(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"Текущее: <code>{read_env_var('rates', '—')}</code>\n\nВведите новое:", reply_markup=kb.as_markup())
    await callback.answer()
...
async def admin_change_sell_btc(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад", callback_data="admin_links")
    await callback.message.edit_text(f"Текущее: <code>{read_env_var('sell_btc', '—')}</code>\n\nВведите новое:", reply_markup=kb.as_markup())
    await callback.answer()
...
async def admin_change_news_channel(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад", callback_data="admin_links")
    await callback.message.edit_text(f"Текущее: <code>{read_env_var('news_channel', '—')}</code>\n\nВведите новое:", reply_markup=kb.as_markup())
    await callback.answer()
...
async def admin_change_operator(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад", callback_data="admin_links")
    await callback.message.edit_text(f"Текущее: <code>{read_env_var('operator', '—')}</code>\n\nВведите новое:", reply_markup=kb.as_markup())
    await callback.answer()
...
async def admin_change_operator2(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад", callback_data="admin_links")
    await callback.message.edit_text(f"Текущее: <code>{read_env_var('operator2', '—')}</code>\n\nВведите новое:", reply_markup=kb.as_markup())
    await callback.answer()
...
async def admin_change_operator3(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад", callback_data="admin_links")
    await callback.message.edit_text(f"Текущее: <code>{read_env_var('operator3', '—')}</code>\n\nВведите новое:", reply_markup=kb.as_markup())
    await callback.answer()
...
async def admin_change_work_operator(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад", callback_data="admin_links")
    await callback.message.edit_text(f"Текущее: <code>{read_env_var('work_operator', '—')}</code>\n\nВведите новое:", reply_markup=kb.as_markup())
    await callback.answer()


# === INPUT handlers for links ===
async def process_env_input_back(message: Message, state: FSMContext, key: str, back_cb: str):
    if not is_admin(message.from_user.id):
        return
