...
VISIBLE_BASE_ENV_KEYS = frozenset({"ADMIN_IDS", "DEFAULT_COMMISSION_PERCENT"})
...
def visible_env_keys(
    ...
    keys.update(f"SELL_WALLET_{key.upper()}" for key in sell_wallet_labels)
...
def apply_runtime_from_env(ctx: AppContext, env: Mapping[str, str | None]) -> None:
    ...
    for wallet_key in ctx.sell_wallet_labels or {}:
        env_key = f"SELL_WALLET_{wallet_key.upper()}"
        if env_key not in env:
            continue
        ctx.settings.set_sell_wallet(wallet_key, (env.get(env_key) or "").strip())
