        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status == 200:
                    ...
                    usd = float(monero.get("usd", 0))
                    rub = float(monero.get("rub", 0))
                    if usd > 0 and rub > 0:
                        print(f"✅ CoinGecko XMR: ${usd}, ₽{rub}")
                        return (usd, rub)
    except Exception as e:
...
async def init_settings_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # ===== НОВОЕ: Проверяем и заполняем пустые payment_methods =====
        ...
        # Если payment_methods пустой — заполняем дефолтными
        if not methods:
            methods = DEFAULT_PAYMENT_METHODS.copy()
            need_update = True
            print("✅ Добавлены дефолтные способы оплаты")
