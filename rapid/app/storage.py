...
from .constants import DEFAULT_PAYMENT_METHODS
...
class OrdersStore:
    ...
    def load(self) -> None:
        ...
        for key, value in raw.items():
            ...
            requisites_sent = item.get("requisites_sent")
            ...
            parsed[order_id] = {
                ...
                "requisites_sent": bool(requisites_sent) if isinstance(requisites_sent, bool) else False,
                "status": cast(
    ...
    def mark_requisites_sent(self, order_id: str) -> bool:
        order = self.data.get(order_id)
        if order is None:
            return False
        if order["requisites_sent"]:
            return False
        order["requisites_sent"] = True
        order["updated_at"] = int(time.time())
        self.save()
        return True
