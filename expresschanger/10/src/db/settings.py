...
default_requisites = os.getenv("REQUISITES", "")
default_bank = os.getenv("BANK", "")
        ...
        if not await cursor.fetchone():
            await db.execute(
                """INSERT INTO settings (id, requisites, bank, payment_methods, commission, requisites_mode) 
                   VALUES (1, ?, ?, ?, ?, 0)""",
                (default_requisites, default_bank, json.dumps(DEFAULT_PAYMENT_METHODS, ensure_ascii=False),
                 DEFAULT_COMMISSION)
            )
            await db.commit()
            return
...
async def get_btc_rates() -> tuple:
    from src.utils.rates import get_btc_rub_rate, get_usd_rub_rate
    try:
        btc_rub = await get_btc_rub_rate()
        usd_rub = await get_usd_rub_rate()
        btc_usd = btc_rub / usd_rub if usd_rub > 0 else 0
        return (btc_usd, btc_rub)
    except Exception as e:
        print(f"Ошибка получения курса BTC: {e}")
        return (68000.0, 68000.0 * 90.0)
