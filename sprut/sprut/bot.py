...
async def process_payment_confirmation(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "payment_paid":
        ...
        if method_id:
            ...
            total_str = str(int(total_amount_rub))
            crypto_str = format_crypto_amount(crypto_amount, currency)
            order_id = create_order(user_id, currency, total_amount_rub, crypto_amount, method_id, wallet_address)

            text = (
                f"🗳 Заявка: №{order_id}\n\n"
                f"⏳ Статус: обрабатывается...\n"
                f"🚀 Будет зачислено {crypto_str} {currency}\n"
...
async def handle_receipt_photo(message: types.Message, state: FSMContext):
    ...
    # Отправляем всем админам из переменной окружения ADMIN_ID
    admin_ids_str = os.getenv("ADMIN_ID", "")
    if admin_ids_str:
        for admin_id_str in admin_ids_str.split(","):
            ...
            if admin_id_str.isdigit():
                await bot.send_photo(
                    ...
                    photo_id,
                    caption=(
                        f"📩 Чек по заявке №{order_id}\n"
                        f"👤 Пользователь: {user_id}"
                    )
                )

    await message.answer("✅ Чек отправлен администратору.")
        ...
        if message.text == "💻 Личный кабинет":
            ...
            if current_state == CaptchaStates.waiting_for_promo:
                ...
                if message.text == "❌ Отмена":
                    ...
                    amount = user_balance / (1 + commission_percent / 100)
                ...
                if message.text == "❌ Отмена":
                    ...
                    try:
                        input_value = parse_amount(message.text)
                        data = await state.get_data()
                        currency = data.get("exchange_currency", "")
                        rates = await get_crypto_rates()
                        rate = rates.get(currency, 9000000)
                ...
                if message.text == "❌ Отмена":
                    ...
                    if requisites:
                        ...
                        text_parts.append(f"На кошелек: <code>{wallet_address}</code>")
                        text_parts.append("")
