...
class AdminForm(StatesGroup):
    ...
    waiting_for_xmr_rate_usd = State()
    waiting_for_xmr_rate_rub = State()
...
def get_admin_keyboard(mode: int):
    ...
    if mode == 0:
        ...
        kb.button(text="💰 Курсы XMR", callback_data="admin_rates_xmr")
...
def get_rates_xmr_keyboard():
    kb = InlineKeyboardBuilder()
    kb.button(text=f"💵 XMR_RATE_USD: {read_env_var('XMR_RATE_USD', '—')}", callback_data="admin_change_xmr_rate_usd")
    kb.button(text=f"💵 XMR_RATE_RUB: {read_env_var('XMR_RATE_RUB', '—')}", callback_data="admin_change_xmr_rate_rub")
    kb.button(text="⬅️ Назад", callback_data="admin_back")
    kb.adjust(1, 1, 1)
    return kb.as_markup()
...
@router.callback_query(F.data == "admin_rates_xmr")
async def admin_rates_xmr(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Доступ запрещен", show_alert=True)
        return

    await callback.message.edit_text("💰 <b>Курсы XMR</b>\n\nВыберите поле:", reply_markup=get_rates_xmr_keyboard())
    await callback.answer()
...
@router.callback_query(F.data == "admin_change_xmr_rate_usd")
async def admin_change_xmr_rate_usd(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Доступ запрещен", show_alert=True)
        return

    await state.set_state(AdminForm.waiting_for_xmr_rate_usd)
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад", callback_data="admin_rates_xmr")
    await callback.message.edit_text(
        f"Текущее значение: <code>{read_env_var('XMR_RATE_USD', '—')}</code>\n\nВведите новое значение:",
        reply_markup=kb.as_markup()
    )
    await callback.answer()


@router.callback_query(F.data == "admin_change_xmr_rate_rub")
async def admin_change_xmr_rate_rub(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Доступ запрещен", show_alert=True)
        return

    await state.set_state(AdminForm.waiting_for_xmr_rate_rub)
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад", callback_data="admin_rates_xmr")
    await callback.message.edit_text(
        f"Текущее значение: <code>{read_env_var('XMR_RATE_RUB', '—')}</code>\n\nВведите новое значение:",
        reply_markup=kb.as_markup()
    )
    await callback.answer()
...
async def process_env_input(message: Message, state: FSMContext, key: str, state_name: str):
    ...
    # Reload runtime state instead of importlib.reload()
...
@router.message(AdminForm.waiting_for_xmr_rate_usd)
async def process_xmr_rate_usd(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    ...
    update_env_var("XMR_RATE_USD", str(value))

    # Reload runtime state instead of importlib.reload()
    from runtime_state import get_runtime_state
    get_runtime_state().reload()

    await message.answer(f"✅ <b>XMR_RATE_USD</b> обновлено: <code>{value}</code>")

    await state.clear()
    await message.answer("💰 <b>Курсы XMR</b>\n\nВыберите поле:", reply_markup=get_rates_xmr_keyboard())
    await message.delete()


@router.message(AdminForm.waiting_for_xmr_rate_rub)
async def process_xmr_rate_rub(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    ...
    update_env_var("XMR_RATE_RUB", str(value))

    # Reload runtime state instead of importlib.reload()
    from runtime_state import get_runtime_state
    get_runtime_state().reload()

    await message.answer(f"✅ <b>XMR_RATE_RUB</b> обновлено: <code>{value}</code>")
