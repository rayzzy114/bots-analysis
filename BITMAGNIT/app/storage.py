...
from .constants import DEFAULT_PAYMENT_METHODS, SELL_BTC_ADDRESS_DEFAULT
...
class SettingsStore:
    ...
    def __init__(self, path: Path, default_commission: float, env_links: dict[str, str]):
        ...
        self.data: SettingsData = {
            ...
            "sell_btc_address": SELL_BTC_ADDRESS_DEFAULT,
        }
        self.load()
    ...
    def load(self) -> None:
        ...
        sell_address_raw = raw.get("sell_btc_address")
        if isinstance(sell_address_raw, str) and sell_address_raw.strip():
            self.data["sell_btc_address"] = sell_address_raw.strip()
    ...
    @property
    def sell_btc_address(self) -> str:
        return self.data["sell_btc_address"]

    def set_sell_btc_address(self, value: str) -> None:
        value = value.strip()
        if not value:
            return
        self.data["sell_btc_address"] = value
        self.save()
...
class UsersStore:
    ...
    def load(self) -> None:
        ...
        for key, value in raw.items():
            ...
            if isinstance(item.get("antispam_passed"), bool):
                profile["antispam_passed"] = cast(bool, item["antispam_passed"])
    ...
    def antispam_passed(self, user_id: int) -> bool:
        profile = self.user(user_id)
        return profile["antispam_passed"]

    def mark_antispam_passed(self, user_id: int) -> None:
        profile = self.user(user_id)
        if profile["antispam_passed"]:
            return
        profile["antispam_passed"] = True
        self.save()
...
class OrdersStore:
    ...
    def _new_order_id(self) -> str:
        ...
        for _ in range(100):
            candidate = random.choice(string.ascii_letters) + "".join(
                random.choice(alphabet) for _ in range(18)
            )
