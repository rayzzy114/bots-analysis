...
class OrdersStore:
    ...
    def latest_order_for_user(
        self,
        user_id: int,
        statuses: set[Literal["pending_payment", "paid", "confirmed", "cancelled"]] | None = None,
    ) -> OrderData | None:
        latest: OrderData | None = None
        for order in self.data.values():
            if order["user_id"] != user_id:
                continue
            if statuses is not None and order["status"] not in statuses:
                continue
            if latest is None:
                latest = order
                continue
            if order["updated_at"] > latest["updated_at"]:
                latest = order
        return latest
